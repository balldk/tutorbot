...
class BootBot extends EventEmitter {
  ...
  sendMessage(recipientId, message, options) {
    ...
    const personaId = options && options.personaId
    const reqBody = {
      recipient,
      message,
  ...
  getUserProfile(userId) {
    const url = `https://graph.facebook.com/v2.12/${userId}?fields=first_name,last_name,profile_pic&access_token=${this.accessToken}`;
    return fetch(url)
      .then(res => res.json())
